...
export type MessagesTimelineRow =
  ...
  | {
      ...
      // T3-CUSTOM(expbkt3): describe durable execution phases.
      label: string;
    }
  | {
      kind: "thinking";
...
export function deriveMessagesTimelineRows(input: {
  ...
  const appendWorkingRow = () => {
    ...
    nextRows.push({
      ...
      // T3-CUSTOM(expbkt3): retain the durable execution label.
  ...
  for (let index = 0; index < input.timelineEntries.length; index += 1) {
    ...
    if (timelineEntry.kind === "work") {
      if (workEntryStaysVisible(timelineEntry.entry) || timelineEntry.entry.tone === "error") {
        nextRows.push({
          kind: "work",
          id: timelineEntry.id,
          createdAt: timelineEntry.createdAt,
