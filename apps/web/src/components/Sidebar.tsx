...
const SidebarThreadRow = memo(function SidebarThreadRow(props: {
  ...
  // T3-CUSTOM(expbkt3): BEGIN — durable execution state outranks the provider-observed status.
...
export default function Sidebar() {
  ...
  // T3-CUSTOM(expbkt3): BEGIN — durable execution recovery actions.
  ...
  const handleThreadContextMenu = useCallback(
      ...
      void (async () => {
        ...
        const supportsTitleRegeneration =
          serverConfigs.get(thread.environmentId)?.environment.capabilities
            .threadTitleRegeneration === true;
        const isRegeneratingTitle = thread.titleRegeneration != null;
        // T3-CUSTOM(expbkt3): surfacing recovery actions once retries are exhausted.
        ...
        const clicked = await settlePromise(() =>
          api.contextMenu.show(
            [
              ...
              ...(recoveryExhausted
                ? [
                    { id: "retry-recovery" as const, label: "Retry recovery" },
                    { id: "dismiss-recovery" as const, label: "Dismiss recovery failure" },
                  ]
                : []),
              // T3-CUSTOM(expbkt3): END
            ],
        ...
        // T3-CUSTOM(expbkt3): BEGIN — durable execution recovery actions.
        if (clicked.value === "retry-recovery") {
        ...
        if (clicked.value === "dismiss-recovery") {
    ...
    [
      ...
      deleteThread,
      // T3-CUSTOM(expbkt3): durable execution recovery actions.
