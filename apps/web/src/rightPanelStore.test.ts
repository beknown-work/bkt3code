...
describe("rightPanelStore", () => {
  ...
  it("drops persisted plan surfaces and does not reopen an empty panel", () => {
    expect(
      ...
      byThreadKey: {
        // T3-CUSTOM(expbkt3): withoutPersistedPlannotatorSurfaces drops thread
        // entries left with no surfaces, so a plan-only panel disappears
        // entirely instead of persisting as closed-and-empty. Observably the
        // same: a missing key reads as a closed, empty panel.
        "env-1:thread-B": {
          isOpen: true,
