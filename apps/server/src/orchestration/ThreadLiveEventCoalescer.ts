...
// T3-CUSTOM(expbkt3): durable execution frames share the bounded live queue.
export type ThreadLiveInput =
  | { readonly kind: "event"; readonly event: OrchestrationEvent }
  | { readonly kind: "synchronized" }
  | Extract<OrchestrationThreadStreamItem, { readonly kind: "execution" }>;
        ...
        Effect.forEach(
          ...
          (input) =>
            Effect.gen(function* () {
              ...
              yield* flushPending();
              // T3-CUSTOM(expbkt3): flush preceding tool updates before execution state.
              if (input.kind === "synchronized" || input.kind === "execution") {
