...
import {
  ...
  // T3-CUSTOM(expbkt3): shared summary prompts for the new provider.
...
import {
  ...
  sanitizePrTitle,
  sanitizeThreadTitle,
  // T3-CUSTOM(expbkt3): shared bounded summary output normalization.
...
export const makeAntigravityTextGeneration = Effect.fn("makeAntigravityTextGeneration")(function* (
  ...
  // T3-CUSTOM(expbkt3): BEGIN — summary helpers use the same isolated, tool-free JSON runner.
  const updateRollingSummary: TextGeneration.TextGeneration["Service"]["updateRollingSummary"] =
    Effect.fn("AntigravityTextGeneration.updateRollingSummary")(function* (input) {
      const generated = yield* runAntigravityJson({
      ...
      return { summary: sanitizeRollingSummary(generated.summary) };
    });

  const generateCatchupSummary: TextGeneration.TextGeneration["Service"]["generateCatchupSummary"] =
    Effect.fn("AntigravityTextGeneration.generateCatchupSummary")(function* (input) {
      const generated = yield* runAntigravityJson({
      ...
      return { summary: sanitizeCatchupSummary(generated.summary) };
    });
  ...
  return {
    ...
    generateBranchName,
    generateThreadTitle,
    // T3-CUSTOM(expbkt3): fork summary capabilities.
