...
// T3-CUSTOM(expbkt3): adapt the new provider to observable durable execution.
...
export const makeAntigravityAdapter = Effect.fn("makeAntigravityAdapter")(function* (
  ...
  const handleEvent = Effect.fn("AntigravityAdapter.handleEvent")(function* (
    ...
    switch (event._tag) {
      ...
      case "ConfigOptionsUpdated":
        ...
        return;
      // T3-CUSTOM(expbkt3): the shared runtime also reports direct child exit.
      case "Exited":
  ...
  const sendTurn: Adapter["sendTurn"] = Effect.fn("AntigravityAdapter.sendTurn")(function* (input) {
    ...
    return yield* Effect.gen(function* () {
      ...
      // T3-CUSTOM(expbkt3): retained history records the fork turn lifecycle state.
      else context.turns.push({ id: launch.turn.turnId, items: [result], state: "in-progress" });
