...
export function makeGrokAdapter(grokSettings: GrokSettings, options?: GrokAdapterLiveOptions) {
  return Effect.gen(function* () {
    // T3-CUSTOM(expbkt3): close disconnected session scopes from outside their event pump.
          ...
          const nf = yield* Stream.runDrain(
            Stream.mapEffect(acp.getEvents(), (event) =>
              Effect.gen(function* () {
                ...
                // T3-CUSTOM(expbkt3): either process death or transport termination settles this generation once.
                if (event._tag === "Exited" || event._tag === "ConnectionTerminated") {
