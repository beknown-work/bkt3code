...
export const make = (options?: StartupOptions) =>
  Effect.gen(function* () {
    ...
    const startup = Effect.gen(function* () {
      ...
      // T3-CUSTOM(expbkt3): A turn cannot outlive the process that ran it: settle
      // any session still marked running/starting by the previous process before
      // commands open. Runs before upstream's provider-session reconciliation so
      // the durable recovery below sees settled projections.
