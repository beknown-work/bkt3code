    ...
    <View className="flex-1 bg-sheet-solid">
      <View className="flex-row items-center justify-between px-4 pt-4">
        ...
        <View className="flex-row items-center gap-4">
          <Pressable accessibilityLabel="Refresh" hitSlop={8} onPress={() => refresh()}>
            {isPending ? (
        ...
        {usage === null ? null : (
          <>
            <View className="mx-4 mt-4 flex-row gap-3 rounded-xl border border-border bg-subtle p-3">
              ...
              <Stat label="Turns" value={String(usage.records)} />
            </View>
              ...
              <>
                ...
                {usage.models.map((row) => (
                  <Row
                    ...
                    sub={`${row.provider.toUpperCase()} · ${formatTokens(tokensOf(row))} tokens · ${row.records} turn${row.records === 1 ? "" : "s"}`}
                  />
                ))}
              </>
            ) : null}
