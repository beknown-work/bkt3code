...
WORKFLOW_RUNS_URL="https://api.github.com/repos/beknown-work/bkt3code/actions/workflows/deploy-alphabkt3.yml/runs?branch=alphabkmain&event=push&per_page=20"

CURRENT_BRANCH="$(git -C "$REPO_DIR" symbolic-ref --quiet --short HEAD 2>/dev/null || true)"
if [[ "$CURRENT_BRANCH" != "$EXPECTED_BRANCH" ]]; then
  echo "ERROR: automatic alphabkt3 deployment requires branch '$EXPECTED_BRANCH' (found '${CURRENT_BRANCH:-detached HEAD}')." >&2
  exit 1
fi
...
echo "GitHub validation succeeded for $REMOTE_SHA; starting alphabkt3 deployment."
exec "$REPO_DIR/deploy/alphabkt3/deploy.sh" "$REMOTE_SHA" "$RUN_ID"
