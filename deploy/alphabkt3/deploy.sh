#!/usr/bin/env bash
# T3-CUSTOM(expbkt3): Verified manual deployment entry point for the alpha branch.
set -euo pipefail
...
SERVICE_NAME="t3-alphabkmain.service"
HEALTH_URL="http://10.31.39.131:18086/"
DEPLOYED_SHA_FILE="/home/ubuntu/.t3/alphabkt3-dev/deployed-sha"

exec 9>"/tmp/alphabkt3-deploy.lock"
if ! flock -n 9; then
  echo "ERROR: another alphabkt3 deployment is already running." >&2
  exit 1
fi
...
ARTIFACT_NAME="alphabkt3-$REMOTE_SHA"
...
LOCK_HASH_FILE="/home/ubuntu/.t3/alphabkt3-dev/deployed-lock"
...
echo "==> alphabkt3 deployed at $DEPLOYED_SHA"
