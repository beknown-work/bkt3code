...
export const makeEnvironmentThreadState = Effect.fn("EnvironmentThreadState.make")(function* (
  ...
  // T3-CUSTOM(expbkt3): disk execution belongs to a previous authority epoch.
  const cachedThread = Option.map(cached, (snapshot) => ({ ...snapshot.thread, execution: null }));
  ...
  const applyItemLocked = Effect.fn("EnvironmentThreadState.applyItemLocked")(function* (
    ...
    if (item.kind === "execution") {
      ...
      if (Option.isSome(current.data)) {
        // T3-CUSTOM(expbkt3): an execution overlay touches only the live
        // execution fields, so the loaded page window must survive it.
        yield* setThread(withExecutionSnapshot(current.data.value, item.execution), "keep");
      }
      return;
    }
