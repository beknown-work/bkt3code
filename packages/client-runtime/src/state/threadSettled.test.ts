...
describe("effectiveSettled", () => {
    ...
    ({ settledOverride, changeRequestState, activityAt, running, pending, expected }) => {
      ...
      expect(
        effectiveSettled(shell, {
          ...
          // T3-CUSTOM(expbkt3): the fork ships with auto-settle-on-merge off.
  ...
  it("never auto-settles on a merge: the merged thread only rejoins the inactivity path", () => {
    ...
    const recentlyActive = makeShell({ activityAt: "2026-04-09T23:59:59.999Z" });
    // T3-CUSTOM(expbkt3): BEGIN — with auto-settle-on-merge off (the fork default) a merge
    // only releases the open-PR blocker.
    expect(
      effectiveSettled(recentlyActive, {
        now: NOW,
        autoSettleAfterDays: 3,
