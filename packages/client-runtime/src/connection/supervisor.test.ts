...
describe("EnvironmentSupervisor", () => {
  ...
  it.effect("reconnects when the periodic heartbeat probe fails", () =>
    Effect.gen(function* () {
      const harness = yield* makeHarness({
        probe: (attempt) =>
          attempt === 1 ? Effect.fail(transient("The idle session went stale.")) : Effect.void,
      });
      const supervisor = yield* EnvironmentSupervisor.make(TARGET_ENTRY, {
        initiallyDesired: true,
      }).pipe(Effect.provide(harness.dependencies));
      ...
      yield* awaitState(supervisor.state, (state) => state.phase === "connected");
      // No wakeup — the heartbeat alone must detect the dead session.
  ...
  it.effect("times out a stalled heartbeat probe and reconnects", () =>
    Effect.gen(function* () {
      const harness = yield* makeHarness({
        probe: (attempt) => (attempt === 1 ? Effect.never : Effect.void),
      });
  ...
  it.effect("keeps a healthy session across periodic heartbeats", () =>
    Effect.gen(function* () {
      const probeCount = yield* Ref.make(0);
      const harness = yield* makeHarness({
      ...
      expect(yield* Ref.get(probeCount)).toBeGreaterThanOrEqual(1);
      expect((yield* SubscriptionRef.get(supervisor.state)).phase).toBe("connected");
      expect(yield* Ref.get(harness.sessionCount)).toBe(1);
      expect(yield* Ref.get(harness.releaseCount)).toBe(0);
    }).pipe(Effect.provide(TestClock.layer())),
  ...
  it.effect("stops the heartbeat after an explicit disconnect", () =>
    Effect.gen(function* () {
      const probeCount = yield* Ref.make(0);
      const harness = yield* makeHarness({
        probe: () => Ref.update(probeCount, (count) => count + 1),
