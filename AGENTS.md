...
4. **Configuration and feature flags over branching.** Fork behavior gated by a flag (`VITE_T3_CONDUCTOR`, a server setting) keeps upstream's code path intact and lets us disable our feature to isolate a regression after a merge.
...
- **Mark every fork edit inside an upstream file** with a `T3-CUSTOM(expbkt3)` comment, so the seam is greppable during a merge and reviewable afterwards. See [expbkt3 customization boundaries](./docs/operations/expbkt3-customizations.md).
